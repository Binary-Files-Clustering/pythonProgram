...
class ClusteringClass(metaclass=abc.ABCMeta):
    ...
    def from_random_vectors(cls, x_min=DEFAULT_X_MIN,
        ...
        """
        list_of_vectors = vector_space.get_n_random_vectors([(x_min, x_max), (y_min, y_max)], n)
    ...
    def random_cluster_assignments(self):
        ...
        """
        for i in xrange(self.vectors_num):
    ...
    def get_cluster(self, cluster_index):
        ...
        """
        return [self.vectors[i] for i in xrange(self.vectors_num) if self.cluster_assignment[i] == cluster_index]

    def get_clusters(self):
        return [self.get_cluster(i) for i in xrange(self.cluster_num)]
...
class KMeans(ClusteringClass):
    ...
    def update_mass_centers(self):
        ...
        for i in xrange(self.cluster_num):
            try:
                self.mass_centers.append(vector_space.get_mass_center(self.get_cluster(i)))
    ...
    def update_vec_cluster_assignment(self, vec_index):
        ...
        """
        self.cluster_assignment[vec_index] = vector_space.get_closest_vec(self.vectors[vec_index], self.mass_centers)
    ...
    def random_mass_centers(self):
        self.mass_centers = vector_space.get_random_mass_centers(self.vectors, self.cluster_num)

    def start(self, random_assignment=False):
        if random_assignment:
            self.random_cluster_assignments()
            self.update_mass_centers()
        else:
            self.random_mass_centers()
